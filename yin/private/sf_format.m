...
fseek(i.fd, 0, -1);
% magic strings?
magic_ = char(fread(i.fd, 4, 'uchar'))';
magic = magic_(isstrprop(magic_,'alphanum'));
if strcmpi(magic,'.snd')
    i.format = 'AU';
	return;
elseif strcmpi(magic,'FORM')
        fseek(i.fd, 8, -1);
...
elseif strcmpi(magic,'NIST')
	...
	disp(i.format);
	return;
elseif strcmpi(magic,'SDIF')
    i.format = 'SDIF';
        return;
elseif strcmpi(magic,'ID3')
    i.format = 'MP3'
    return;
elseif contains(magic,[char(255) char(242)])
    i.format = 'MP3'
    return;
elseif contains(magic,[char(255) char(243)])
    i.format = 'MP3'
    return;
elseif contains(magic,[char(255) char(251)])
    i.format = 'MP3'
    return;
end
% wff magic number?
