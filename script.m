clear all
clc
fileExtension = '.wav'; % replace this with e.g. 'wav' if needed
addpath('yin')
...
figure
tiledlayout('flow')
for k = 1:size(out,2),nexttile,plot(out{k}.pitch{5}),title(strrep(out{k}.filename,fileExtension,'')),hold on,plot(out{k}.temp{5}),title(strrep(out{k}.filename,fileExtension,'')),axis tight,end
